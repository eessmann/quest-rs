...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Track all library paths for rpath
    ...
    if let Some(ref quest_dir) = quest_dir {
        ...
        // Add lib directory to library_paths for rpath
        let lib_dir = path.join("lib");
        if lib_dir.exists() {
            library_paths.insert(lib_dir.to_string_lossy().to_string());
        }
    }
                        ...
                        if let Some(objects) = index_data["objects"].as_array() {
                            for obj in objects {
                                if let Some(kind) = obj["kind"].as_str() {
                                    if kind == "codemodel" {
                                        if let Some(json_file) = obj["jsonFile"].as_str() {
                                            return reply_dir.join(json_file);
                                        }
                                    }
                                }
                            }
    ...
    // Add rpath flags for each library path we found
            ...
            if let Some(libraries) = link.get("libraries").and_then(|v| v.as_array()) {
                ...
                for lib in libraries {
                    if let Some(lib_name) = lib.get("name").and_then(|v| v.as_str()) {
                        ...
                        process_library_name(lib_name, is_windows, library_paths);
                    }
                }
            } else {
            ...
            if let Some(lib_paths) = sysroot_link.get("libraryDirectories").and_then(|v| v.as_array()) {
                for path in lib_paths {
                    if let Some(path_str) = path.get("path").and_then(|v| v.as_str()) {
                        ...
                        println!("cargo:rustc-link-search=native={}", path_str);
                        library_paths.insert(path_str.to_string());
                    }
                }
            }
        }
...
fn process_library_name(lib_name: &str, is_windows: bool, library_paths: &mut HashSet<String>) {
